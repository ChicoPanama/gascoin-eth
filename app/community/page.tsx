...
export default async function Community(){
  ...
  return <>
    ...
    <p>Verified refunds paid out to the community.</p>
    {claims.length === 0 ? (
      <div className="empty-state">
        <p>No refunds have been paid out yet. Be the first to submit a claim!</p>
      </div>
    ) : (
      <table className="table">
        <thead><tr><th>X Account</th><th>Tweet</th><th>Amount (SOL)</th><th>Tx Hash</th><th>Status</th></tr></thead>
        <tbody>{claims.map((c:any)=>(
          <tr key={c.id}>
            <td>{c.xHandle}</td>
            <td><a href={c.tweetUrl} target="_blank" rel="noopener">View</a></td>
            <td className="mono">{c.amountSol}</td>
            <td>{c.txHash ? <a href={c.txUrl} target="_blank" rel="noopener" className="mono truncate">{c.txHash}</a> : '--'}</td>
