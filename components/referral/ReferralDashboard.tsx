...
/**
 * Full referral dashboard — extracted from the former /referral page so it
 * can be embedded inside /me as an expanded section. Identical behavior to
 * the standalone page; the optional `showHeader` prop toggles the large
 * REFERRAL ENGINE title that appears on the dedicated /referral URL.
 */
...
export interface ReferralDashboardProps {
  /** When true, renders the dedicated-page header ("REFERRAL ENGINE"). Embeds (e.g. /me) set false. */
  showHeader?: boolean;
}

export function ReferralDashboard({ showHeader = false }: ReferralDashboardProps) {
  const { address } = useGascoinWallet();
  const wallet = address ?? null;
  const { summary, conversions, clickStats, referralCode, hasApprovedSubmission, loading, error } =
    useReferralDashboard(wallet);
          ...
          <div className="cf-filters" style={{ marginBottom: 16 }}>
            {(['all', 'dispatched', 'pending', 'skipped'] as ConvFilter[]).map((f) => (
              <button
                key={f}
                type="button"
                className={`cf-filter-tab${convFilter === f ? ' cf-filter-tab--active' : ''}`}
              ...
              <table className="lb-table">
                ...
                <tbody>
                  {paged.map((c: ReferralConversion) => (
                    <tr
                      key={c.id}
                      className={`lb-table-row${c.reward_status === 'skipped' ? ' wt-row-rejected' : ''}`}
