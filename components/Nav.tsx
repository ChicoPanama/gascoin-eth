...
// Full link set — shown on large desktops (>1600px)
const FULL_LINKS = [
...
// Minimal link set — shown on laptops and smaller (hamburger has full set)
const COMPACT_LINKS = [
  { href: '/how-it-works', label: 'How It Works', icon: '?' },
  { href: '/leaderboard', label: 'Leaderboard', icon: '▲' },
];
...
export function Nav() {
  ...
  return (
    <nav ref={navRef as any} className={`gc-nav${compact ? ' gc-nav--compact' : ''}`}>
      ...
      {/* Full links shown on desktop (>1600px) */}
      ...
      {/* Compact links shown on laptops/mobile (≤1600px) */}
