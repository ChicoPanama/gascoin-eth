...
describe('CATEGORY 8: Tier + Cooldown Interaction', () => {
  ...
  it('Test 38: Tier boundaries are EXCLUSIVE — 4,999,999 is Commuter not Road Warrior, 9,999,999 is Road Warrior not Fleet', () => {
    ...
    expect(justBelowCommuter.max_eth_refund).toBe(0.004);  // Standard cap
    expect(justBelowRoadWarrior.max_eth_refund).toBe(0.01); // Commuter cap
    expect(justBelowFleet.max_eth_refund).toBe(0.02);       // Road Warrior cap
  });
});
