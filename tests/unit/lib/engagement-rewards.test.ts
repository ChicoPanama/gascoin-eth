...
describe('calculateEngagementPoints', () => {
  ...
  it('weights quote tweets highest', () => {
    const qt = calculateEngagementPoints({ impressions: 0, likes: 0, retweets: 0, quote_tweets: 1, replies: 0, bookmarks: 0 });
    const rt = calculateEngagementPoints({ impressions: 0, likes: 0, retweets: 1, quote_tweets: 0, replies: 0, bookmarks: 0 });
    const like = calculateEngagementPoints({ impressions: 0, likes: 1, retweets: 0, quote_tweets: 0, replies: 0, bookmarks: 0 });
...
describe('calculateFullBreakdown', () => {
  it('sums all point sources', () => {
    const b = calculateFullBreakdown({
      impressions: 1000, likes: 10, retweets: 5, quote_tweets: 2, replies: 3, bookmarks: 1,
    ...
    expect(b.totalPoints).toBe(
      ...
      b.replyPoints + b.bookmarkPoints + b.submissionPoints + b.streakPoints + b.referralPoints + b.holdingsPoints
    );
  });

  it('includes holdings bonus', () => {
    const b = calculateFullBreakdown({
      impressions: 0, likes: 0, retweets: 0, quote_tweets: 0, replies: 0, bookmarks: 0,
  ...
  it('has no SOL conversion — points only', () => {
    const b = calculateFullBreakdown({
      impressions: 100000, likes: 1000, retweets: 500, quote_tweets: 100, replies: 200, bookmarks: 50,
